...
export const PromptBuilder: React.FC<PromptBuilderProps> = ({
  ...
  const renderSelector = (sectionId: string, type: 'mood' | 'gender' | 'theme' | 'timestamp' | 'bpm' | 'scale') => {
    ...
    const configData = PROMPT_HELPER_CONFIG as any;
    const options: string[] = type === 'mood' ? configData.moods : 
                    type === 'gender' ? configData.genders : 
    ...
    return (
      <div className="absolute top-full left-0 mt-2 z-[100] bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 min-w-[200px] animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
        <div className="space-y-1 max-h-48 overflow-y-auto custom-scrollbar">
          {options.map(opt => {
            const tooltipText = configData.tooltips && configData.tooltips[type + 's'] ? configData.tooltips[type + 's'][opt] : null;
            return (
            <button 
              key={opt} 
              title={tooltipText || ''}
              ...
              className={`w-full text-left px-4 py-2 flex flex-col justify-center rounded-xl text-sm transition-colors ${currentValue === opt ? 'bg-blue-50 text-blue-600 font-bold' : 'hover:bg-gray-50 text-gray-700'}`}
            >
              <span>{opt}</span>
              {tooltipText && <span className="text-[10px] font-normal text-gray-400 mt-0.5 line-clamp-1">{tooltipText}</span>}
            </button>
          )})}
        </div>
